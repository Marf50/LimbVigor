...
v1.24 tucks **LifeBar10 after Hunger** on a Dark UI–exact MainPanel (workshop 1200632417). MedicalPanel is not scaled. v1.23 (`2bc122a`) grew MedicalPanel and drifted the whole HUD. Widget::setCaption (QEAAX) writes Hemolymph on LifeBar10 **after orig every selected-person tick**; `setVisible` is LifeBar10 / LifeBar10Value only. **No `_getWidget`.** See [testdoc/HUD.md](testdoc/HUD.md).
...
v1.24: Dark UI exact + LifeBar10 tucked, no parent scale. v1.23 full-layout drift is rejected. `46d810a` LifeBar1 overwrite is rejected.
