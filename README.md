...
v1.31 puts Hemolymph on **LifeBar10 inside MedicalPanel** (sibling of Back, tucked under Hunger with the Blood→Head gap, drawn after Front). MedicalPanel / Back / Front pixel sizes match **`3b999b9`**. LifeBar10Datapanel is **PanelEmpty** like LifeBar9. Write: hide stun skins, **pixel** Green fill from the LifeBar10 **host** after show (50–400px), **Hemolymph on Datapanel**. Value stays the number. A Left Leg at 5 stays a stump and **flesh HP moves** — **no restore-on-stump / Equip GROWN**. Settings-close drops the LifeBar10 cache. Widget::setCaption is not exported. **No `_getWidget`.** See [testdoc/HUD.md](testdoc/HUD.md).
...
v1.31: tight stack + green fill + stump HP + settings-close safe. Root-level HemolymphStrip is rejected. Stretching the 9-bar skin is rejected. Widget::setCaption hunt is rejected. Restore-on-stump / Equip GROWN is rejected. TextBox Datapanel is rejected. v1.23 full-layout drift is rejected. `46d810a` LifeBar1 overwrite is rejected.
