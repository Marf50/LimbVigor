...
Resolve, paint, and lifetime/invalidate live in `src/lv_hud.cpp` + `src/lv_hud.h`. The MyGUI export bind/dump lives there too. `src/lv_game.cpp` is world / medical / stump / MainBar helpers only. Do not grow another 4k-line HUD wall inside `lv_game.cpp`.

## Lifetime (v1.39) — not folklore

**v1.38 FAIL:** `findWidgetT(OptionsPanel / OptionsWindow / odSettingsOpen / Kenshi_Options)` never entered freeze. Options layout loaded, MainPanel reloaded, `LifeBar11 not found`, TTF rebuilt, crash. `odSettingsOpen` is a dump string, not a widget. Name-guess is **deleted**.

**Real signal:** a cached `LifeBar10*` is dead — `getName` SEH, `getVisible` SEH, or the name no longer ends with `LifeBar10`. On that: null **all** MyGUI pointers (`LifeBar10*`, `LifeBar9*`, Root, MedicalPanel, Front, Back, caption dest), drop the prefix, skip writes this tick, re-resolve next tick after MainPanel exists again. Log once per death: `HUD invalidate (widget died)`. Cache across ticks is allowed only after that probe succeeds. We do not hook ESC, the RE_Kenshi injected settings button, `eventFrameStart`, `createWidget`, `_getWidget`, WindowCX, ctor `0x72C1E0`, or epoch-drop.

Why ESC close will not UAF: we never keep a MyGUI pointer across a tick where the widget failed a name/vis probe. Options reload frees MainPanel; the next getName/getVisible SEH or wrong name invalidates the whole cache before any setCaption/setSize/setVisible. We do not guess Options widget names.
...
- Vanilla and Dark UI `Kenshi_MainPanel.layout` **stop at LifeBar9**. They omit **LifeBar10** and **LifeBar11**. Kenshi `BaseLayout` still `assignWidget`-s them: after we added 10 the reload dump warned `LifeBar11 not found`. **v1.39** ships a bind-safe **LifeBar11** stub (Root child, `position_real="0 0 0 0"`, `Visible=false`, Depth=0) so assignWidget has the name. LifeBar11 is **not** a visible Hemolymph row. No `LifeBar11Value` / `LifeBar11Tooltip` — those names were not shown as MainBar assignWidget targets. Do **not** skin-stretch 1–9. Do **not** wrap in `LifeBar10Slot`.
...
- v1.39: HUD split + invalidate on dead LifeBar10* + LifeBar11 bind stub. Bar stays. Ready: `ready v1.39 — HUD lifetime + LifeBar11 bind, bar stays, ESC close must live`.
