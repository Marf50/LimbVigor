...
## H2 ownership — why ESC can live (not "we stopped writing")

**Unbound-slot baseline (Dylan live pack, confirmed on disk):** his live `Kenshi_MainPanel.layout` is **byte-identical to vanilla**. LifeBar1–9 only. No LifeBar10, no LifeBar11, no Hemolymph. MedicalPanel `position_real=0.126042 0.712963 0.182292 0.3` (y=`0.712963`). That tree leaves Kenshi's LifeBar10 assignWidget slot **null**. That is the class of layout Kenshi survives on MainPanel reload. H2 and H3 confirmed. Hidden LifeBar11 is not required.

**Do not rebase our override onto that vanilla 1–9 / MedicalPanel.** That would trash the Dark UI look. Our override keeps Dark UI unstretched 1–9, MedicalPanel y **locked** `0.70473849555969238` (not vanilla `0.712963`), and `HemolymphBar` / `HemolymphValue` / `HemolymphTooltip` at the **current** v1.33 Root-after-MedicalPanel visible coords (today's old LifeBar10 place). HemolymphBar is ours. Internal names: no `LifeBar10*` / `LifeBar11*`.

`LifeBar10` is Kenshi's `assignWidget` slot, not our product name. Vanilla and Dark UI **stop at LifeBar9** and never instantiate `LifeBar10`, so MainBar's LifeBar10* stays **null** across Options MainPanel reload and ESC lives (Dark UI has warned `LifeBar10 not found` for years). When we shipped a real widget named `LifeBar10`, assignWidget held that pointer; reload could UAF on **Kenshi's own member** even if LimbVigor never wrote. `LifeBar11 not found` after we added 10 is walk-until-first-miss — **not** proof we must add 11. Do **not** add LifeBar11.

Our row is **HemolymphBar** (Root child after MedicalPanel, same v1.33 `position_real` / Depth=1 as the old LifeBar10). Children: `HemolymphBarGrey/Red/Yellow/White/Green/Robot/Crushed/Datapanel`. Number: `HemolymphValue`. Tooltip: `HemolymphTooltip`. The player never sees those names. On screen it must still look like Blood: tight under Hunger, same plate, centered Hemolymph/Vigor/Battle-heat, number on the right, green fill when full. Person only. Door clears our bar only. **Bar gone after ESC = fail**, even if Kenshi lives.

CI: fail if `name="HemolymphBar"` is missing. Fail if `name="LifeBar10"` exists (we occupied the slot again). Do not fail if LifeBar11 is missing.

## Options layout — proven names, never guess

`Kenshi_OptionsPanel.layout` widgets (Dylan live pack): **Root, OptionsTab, CloseButton, DefaultButton, TooltipPanel**. There is **no** widget named `OptionsPanel`. That is why v1.38 freeze never entered. Do **not** guess `OptionsPanel` / `OptionsWindow` / `odSettingsOpen` / `Kenshi_Options` ever again. Options name-guess is deleted.

## Lifetime (H1 seatbelt) — not folklore
...
H1 is the seatbelt. H2 is Kenshi's LifeBar10* staying null like Dark UI.
...
- Vanilla and Dark UI omit **LifeBar10** and **LifeBar11**. That empty LifeBar10 name is what we must keep. Our extra row is **HemolymphBar**, not LifeBar10.
- LimbVigor ships `kenshi_mod/gui/layout/Kenshi_MainPanel.layout` started from workshop **1200632417** Dark UI. **Do not rebase this override onto vanilla 1–9 or vanilla MedicalPanel** (`0.126042 0.712963 0.182292 0.3`). **MedicalPanel y STAYS** our locked `0.70473849555969238`. x/w/h stay byte-exact. Back / Front / LifeBar1–9 stay byte-exact `4ea66b2` / `3b999b9` (unstretched). Front stays `0 0 0.99665552377700806 1.0030864477157593` `Depth=0`. **v1.34 `LifeBar10Slot` BottomSkin 1-row made getW=2 / invisible** — do **not** reintroduce it. **HemolymphBar** + Value + Datapanel + Tooltip are **Root children after MedicalPanel** at the **current** v1.33 screen coords (`HemolymphBar` `0.14218749750845783 0.9797385139738242 0.12395832622918707 0.02129629746523392`, Depth=1) — same look as the old LifeBar10, different internal name. Datapanel stays PanelEmpty `0 0.1304347813129425 0.99579828977584839 0.73913043737411499` as a child of HemolymphBar. Hunger→Hemolymph gap stays 0.000926. Size Green from a host whose getW is 50–400. If HemolymphBar **and** LifeBar9 getW≤14, use layout Hunger pixels (`Green host fallback layout Hunger parentW=NNN`) — never size from 2. Do **not** copy LifeBar9Datapanel if 0x0; setSize host W × Hunger H, then ISub Hemolymph. Do **not** stretch the 9-row Back/Front. No `HemolymphStrip`. MedicalPanel stays left of Floor / Speed. Touch no non-medical chrome.
...
- v1.37 playtest FAIL (Dylan): ESC/settings close still dies. `Injected settings button` → Kenshi_OptionsPanel.layout → Kenshi_MainPanel.layout **RELOAD** (`LifeBar11 not found`) → every TTF rebuilt → destructor / Crash. ZERO LimbVigor lines. No `HUD SEH skip`. Dump `odSettingsOpen`. Heap/reload death — cached LifeBar10* UAF while MyGUI destroys MainPanel. LV_TRY on setCaption never ran.
- v1.38: **HUD freeze across Options reload.** Detect OptionsPanel / OptionsWindow / odSettingsOpen / Kenshi_Options by Gui findWidgetT (not ESC hook, not injected settings button, not Paused/Settings vis=1). Immediately null **all** MyGUI pointers (LifeBar10*, LifeBar9*, Root, MedicalPanel, Front, Back, caption). While frozen: no find/setCaption/setSize/setVisible/setDepth/getW/paint. Stay frozen until Options layout is gone **and** LifeBar10* re-resolve succeeds. Logs once: `HUD freeze (MainPanel reload)` / `HUD thaw`. Layout left 1.37 byte-exact. Ready: `ready v1.38 — HUD freeze across Options reload, ESC close must live`.
...
- v1.39 work: HUD module split. Options name-guess deleted. **Do not probe a cached MyGUI pointer** (getName on a freed LifeBar10* is the bug). Re-find HemolymphBar* every after-orig tick. **H2:** do not occupy Kenshi's LifeBar10 assignWidget slot — rename the visible row to HemolymphBar at the same v1.33 Root coords. LifeBar11 is **rejected** (walk-until-first-miss). Dylan live pack (byte-identical vanilla, LifeBar1–9 only, MedicalPanel y=`0.712963`) is the unbound-slot class Kenshi survives; we do **not** rebase our Dark UI override onto it. No ready line until Dylan/CoS say ESC lives because Kenshi's LifeBar10* stays null like that live tree.

## Landmines

No runtime `createWidget`. No HemolymphStrip. No `LifeBar10Slot`. No widget named `LifeBar10` / `LifeBar10*` / `LifeBar11` / `LifeBar11*`. No MainBar ctor `0x72C1E0`. No `eventFrameStart`. No TitleScreen MyGUI. No `ForgottenGUI::changeFontSize`. No DatapanelGUI `_NV_update` / `_NV_setObject`. No `GetRealAddress` on virtuals. No Gui tree walk. No Goal/State paint. No WindowCX. No `setSize` except HemolymphBarGreen / a 0-size HemolymphBarDatapanel. No `_getWidget`. No `setVisible` on Root / MedicalPanel / Back / Front / LifeBar1–9. No Datapanel / `setLineProgress` HUD path. No unlogged Equip throw. No ESC/pause/options hooks. No epoch-drop. No `restore-on-stump skipped … no write`. No PausedPanel/Options vis=1 skip-paint. No OptionsPanel / OptionsWindow / odSettingsOpen / Kenshi_Options name-guess. No getName/getVisible on yesterday's pointer. No rebase of this override onto vanilla MedicalPanel y `0.712963` / vanilla 1–9. No per-tick HUD log spam (once-per-resolve / 15s / on-change / invalidate once per death). Do not grow HUD state back into `lv_game.cpp`.
...
- v1.39 work: HUD split + re-find HemolymphBar each paint + LifeBar10 slot left empty (H2). LifeBar11 rejected. Live-pack vanilla tree is the unbound-slot class; override stays Dark UI (MedicalPanel y `0.70474`, Hemolymph* at current coords). No ready line from this cut.
