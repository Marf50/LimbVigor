...
**v1.38 FAIL:** `findWidgetT(OptionsPanel / odSettingsOpen)` never entered freeze. `odSettingsOpen` is a dump string. Probing a cached LifeBar10* with getName/getVisible is still a UAF — the crash is on the settings-button / MainPanel-reload / TTF / destructor stack with zero LimbVigor lines.
...
- v1.38 playtest FAIL: OptionsPanel detect never fired (`odSettingsOpen` is not a widget). Crash on MainPanel reload.
- v1.39 work: HUD split + re-find HemolymphBar each paint + LifeBar10 slot left empty (H2). LifeBar11 rejected. No ready line from this cut.
