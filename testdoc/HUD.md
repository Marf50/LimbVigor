...
- LimbVigor ships `kenshi_mod/gui/layout/Kenshi_MainPanel.layout` started from workshop **1200632417** Dark UI. **v1.33:** MedicalPanel y nudged up `0.71073849555969238 - 0.006` = `0.70473849555969238`. x/w/h stay byte-exact. Back / Front / LifeBar1–9 stay byte-exact `dff5e0a` / `3905fa8` / `3b999b9`. Front stays `0 0 0.99665552377700806 1.0030864477157593` `Depth=0`. **v1.32 last-child-of-Front failed** (still under chrome). LifeBar10 + Value10 + Tooltip10 are **Root children after MedicalPanel**, Depth=1 (above Front). Hunger→Hemolymph screen gap stays 0.000926. **LifeBar10Datapanel matches LifeBar9Datapanel** (`Widget` / `PanelEmpty` / `0 0.1304347813129425 0.99579828977584839 0.73913043737411499`). Do **not** stretch BottomSkin/TopSkin. No `HemolymphStrip`. MedicalPanel stays left of Floor / Speed. Touch no non-medical chrome.
