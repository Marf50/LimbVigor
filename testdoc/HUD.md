...
Do **not** ship a layout that aims at these fails: bar gone after ESC; chopped/dashed leftover-pad chrome instead of a real bar; an 11th visible vitals bar or a second Hemolymph. No `LifeBar10Slot` / `HemolymphStrip`. No `LifeBar10` / `LifeBar11` names. If 1–9 numbers drift, STOP — do not "fix" by stretching.

Children/skins stay today's HemolymphBar (Grey/Red/Yellow/White/Green/Robot/Crushed/Datapanel PanelEmpty). Internal names: no `LifeBar10*` / `LifeBar11*`. Do not rename again.

CI: fail if `name="HemolymphBar"` is missing. Fail if `name="LifeBar10"` exists. Do not fail if LifeBar11 is missing.
...
- LimbVigor ships `kenshi_mod/gui/layout/Kenshi_MainPanel.layout` started from workshop **1200632417** Dark UI. **Do not rebase this override onto vanilla 1–9 or vanilla MedicalPanel** (`0.126042 0.712963 0.182292 0.3`). **MedicalPanel y STAYS** our locked `0.70473849555969238`. x/w/h stay byte-exact. Back / Front / LifeBar1–9 stay byte-exact `4ea66b2` / `3b999b9` (unstretched). **Do not grow Back/Front.** Front stays `0 0 0.99665552377700806 1.0030864477157593` `Depth=0`. **v1.34 `LifeBar10Slot` BottomSkin 1-row made getW=2 / invisible** — do **not** reintroduce it. **HemolymphBar** is last child of `MedicalPanel_Back` AFTER LifeBar9 at locked `0.0071942447684705257 0.9008264392614365 0.85611510276794434 0.095041319727897644` (leftover pad; same x/w/h as LifeBar9). **HemolymphValue** / **HemolymphTooltip** last on Front after LifeBar9Value / LifeBar9Tooltip. **No Hemolymph* on Root.** Datapanel stays PanelEmpty `0 0.1304347813129425 0.99579828977584839 0.73913043737411499` as a child of HemolymphBar. Size Green from a host whose getW is 50–400. If HemolymphBar **and** LifeBar9 getW≤14, use layout Hunger pixels (`Green host fallback layout Hunger parentW=NNN`) — never size from 2. Do **not** copy LifeBar9Datapanel if 0x0; setSize host W × Hunger H, then ISub Hemolymph. Do **not** stretch the 9-row Back/Front. No `HemolymphStrip`. MedicalPanel stays left of Floor / Speed. Touch no non-medical chrome. Known chrome risk: leftover pad of the 9-slot skin (v1.27/v1.32). Do not wrap it.
