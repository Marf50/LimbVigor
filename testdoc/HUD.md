...
- LimbVigor ships `kenshi_mod/gui/layout/Kenshi_MainPanel.layout` started from workshop **1200632417** Dark UI. **v1.32:** MedicalPanel y slid +0.02222 (`0.68852→0.71074`), x/w/h stay `3b999b9`. Back / Front / LifeBar1–9 stay byte-exact `3b999b9`. **Sibling-after-Front failed** because Front `Kenshi_HealthPanelTopSkin` has `Depth=0` and covers the unused pad. LifeBar10 + Value10 + Tooltip10 are **last children of Front** (coords remapped into Front space). Hunger→Hemolymph screen gap stays 0.000926. **LifeBar10Datapanel matches LifeBar9Datapanel** (`Widget` / `PanelEmpty` / `0 0.1304347813129425 0.99579828977584839 0.73913043737411499`). Do **not** stretch BottomSkin/TopSkin. No `HemolymphStrip`. MedicalPanel stays left of Floor / Speed. Touch no non-medical chrome.
