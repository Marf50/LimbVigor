...
- LimbVigor ships `kenshi_mod/gui/layout/Kenshi_MainPanel.layout` started from workshop **1200632417** Dark UI. **v1.29/v1.30:** MedicalPanel grows one Hunger-slot (`h 0.300→0.3213`, y `0.68852→0.66722`, bottom ≈0.988). Back/Front **fractions shrink** by `oldH/newH` so their **pixel size matches `3b999b9`**. LifeBar1–9 `position_real` **of Back** stay exact `3b999b9`. LifeBar10 is a **child of MedicalPanel, sibling of Back**, in the new space under the unstretched 9-bar (not inside Back, not a Root-level strip). **LifeBar10Datapanel matches LifeBar9Datapanel** (`Widget` / `PanelEmpty` / same `position_real`). Do **not** stretch BottomSkin/TopSkin (v1.27). No `HemolymphStrip` (v1.28 rejected). No TextBox Datapanel (v1.29 rejected). Touch no non-medical chrome.
