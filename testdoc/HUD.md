...
- LimbVigor ships `kenshi_mod/gui/layout/Kenshi_MainPanel.layout` started from workshop **1200632417** Dark UI. **v1.29:** MedicalPanel grows one Hunger-slot (`h 0.300→0.3213`, y `0.68852→0.66722`, bottom ≈0.988). Back/Front **fractions shrink** by `oldH/newH` so their **pixel size matches `3b999b9`**. LifeBar1–9 `position_real` **of Back** stay exact `3b999b9`. LifeBar10 is a **child of MedicalPanel, sibling of Back**, in the new space under the unstretched 9-bar (not inside Back, not a Root-level strip). Do **not** stretch BottomSkin/TopSkin (v1.27). No `HemolymphStrip` (v1.28 rejected). Touch no non-medical chrome.
...
- v1.29: LifeBar10 in MedicalPanel under Back. Green bind is a **real int** `getWidth` / `ICroppedRectangle` / `getSize` / `getCoord` ref. Accept only **50–400**. `painted=0` if `parentW>1000` or `green w≤4`. getSubWidgetText then ISub on Datapanel (now a TextBox); read-back must be Hemolymph. Value is the number. Stump detector reads `HealthPartStatus::flesh` (named leftLeg/rightLeg/… first). Official STUMP/CRUSHED, flesh≤0 (crippled), or cut-off nub (`hp<10`). Intact 75-HP arms are WHOLE. Per-limb SEH around `LvSyncOneLimb` so one AV does not skip the tick. Log limb + hp + state + why.
