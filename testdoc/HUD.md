...
- LimbVigor ships `kenshi_mod/gui/layout/Kenshi_MainPanel.layout` started from workshop **1200632417** Dark UI. **MedicalPanel y STAYS** `0.70473849555969238` (v1.33 nudge). x/w/h stay byte-exact. Back / Front / LifeBar1–9 stay byte-exact `4ea66b2` / `3b999b9`. Front stays `0 0 0.99665552377700806 1.0030864477157593` `Depth=0`. **v1.33 Root-after-MedicalPanel was flat** — LifeBar10 sat outside `Kenshi_HealthPanelBottomSkin`, so Dylan saw a flat fill and no left label. **v1.34:** wrap LifeBar10 in a **1-row Hunger-height chrome** named `LifeBar10Slot` (Root child after MedicalPanel, skin `Kenshi_HealthPanelBottomSkin`, `0.14114583087146076 0.9788125891584537 0.1447916592388241 0.02222222541088329`, Depth=1). LifeBar10 is a **child of LifeBar10Slot** (same relative x/w as 1–9 inside Back). Value10 / Tooltip10 stay Root siblings, Depth=2, so they paint on top of that chrome. Hunger→Hemolymph screen gap stays 0.000926. Size LifeBar10Datapanel from **LifeBar9Datapanel pixels** (copy that widget’s laid-out w/h, not 0x0); ISub/`getSubWidgetText` must read back Hemolymph. Value 99 stays. Do **not** stretch the 9-row Back/Front. No `HemolymphStrip` / `FloatingPanelSkin`. MedicalPanel stays left of Floor / Speed. Touch no non-medical chrome.
