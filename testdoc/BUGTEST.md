...
**Where to look:** select a **character** after In-game. v1.19 paints on **LifeBar1Datapanel only**, found with MainBar `_getWidget` (RVA `0x723780`). RE_Kenshi_log.txt should list `_getWidget 'LifeBar1'` / `LifeBar1Datapanel` / `LifeBar1Value` / `LifeBar1Green` / `HealthText` / `MedicalPanel` with ptr/vis/name, plus a BaseLayout prefix if readable. Then either `setLineProgress Hemolymph` (or Vigor / Battle-heat) on that panel, or a why-line if it cannot take a line. Must NOT use `findWidgetT("LifeBar1")` as the find path (those names are prefixed). Must NOT call MainBar ctor `0x72C1E0`. Must NOT walk the Gui tree. Must NOT treat medicalPanel / MainBar+0x188 as DatapanelGUI. Must NOT paint Goal/State. No createWidget. No `setCaption` on LifeBar1Value. No LifeBar1Green fill write.
...
- [ ] **_getWidget LifeBar1* + paint there only.** Select a **person**. Log has `_getWidget 'LifeBar1Datapanel'` with a non-null ptr (v1.18 findWidgetT was all null). Hemolymph / Vigor / Battle-heat appears on that overlay if `setLineProgress` takes a line; otherwise a why-line and no Goal/State paint. Door / animal clears those two lines. Blood number (LifeBar1Value) and LifeBar1Green fill stay the game's. Send `RE_Kenshi_log.txt`.
