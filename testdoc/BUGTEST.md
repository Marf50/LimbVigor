...
**Where to look:** select a **character** after In-game. v1.20 must **not crash**. Log MainBar ptr, `proven=1` (`*(bar+0x188)==medicalPanel`), lookup of MedicalPanel / LifeBar1 / LifeBar1Datapanel, any SEH, and `painted=0` or `painted=1`. If `GUI probe SEH` appears, `_getWidget` must **not** run again. Must NOT treat +0x40 as a C string. Must NOT compute MainBar = medicalPanel-0x188. Must NOT call MainBar ctor `0x72C1E0`. Must NOT walk the Gui tree. Must NOT paint Goal/State. See testdoc/HUD.md.
...
- [ ] **Person-select does not kill the process.** Select a **person**. Game stays up. Log has MainBar proven (or a skip), LifeBar1* lookup, and either `painted=1` or `painted=0` / a why-line. Must NOT retry `_getWidget` after `GUI probe SEH`. Door / box / chair still no crash. Send `RE_Kenshi_log.txt`.
