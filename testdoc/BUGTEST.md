...
- [ ] **Reaches the menu.** `ready v1.18 — findWidget LifeBar1Datapanel, setLineProgress there only, no tree walk` then `Main menu loaded`. Must NOT log `HUD created at title screen` or `Blood HUD created after in-game`. Must NOT die after title create / at ~12s with `Log manager destructor`. Must NOT hook MainBarGUI ctor / changeFontSize / `_NV_update` / `_NV_setObject`. Must NOT walk the Gui tree.
...
- [ ] **findWidget LifeBar1* + paint there only.** Select a **person**. Log has `findWidget 'LifeBar1Datapanel'` (and the other LifeBar1* names). Hemolymph / Vigor / Battle-heat appears on that overlay if `setLineProgress` takes a line; otherwise a why-line and no Goal/State paint. Door / animal clears those two lines. Blood number (LifeBar1Value) and LifeBar1Green fill stay the game's. Send `RE_Kenshi_log.txt`.
