...
**Where to look:** select a **character** after In-game. A **new row under Hunger** (LifeBar10) must sit **on the grey plate** and show **Hemolymph** on the bar (Datapanel) and a **number** in the right column (Value). Green fill matches hemolymph. Blood stays Blood. 1–9 stay the same pixel size. Name / Squad / Floor / speed chrome must not drift. Log: `setCapW=0` expected. `setCapISub=1`. Green `setSize`/`setCoord`. After orig `vis10=1`. `painted=1` only if vis=1 and Datapanel is Hemolymph/Vigor/Battle-heat and Value is a number and green w>0. Must NOT write the word to Value. Must NOT call `_getWidget`. Door hides/clears LifeBar10* only. See testdoc/HUD.md.
...
- [ ] **Reaches the menu.** `ready v1.27 — MedicalPanel grow + Hemolymph on Datapanel + Green fill` then `Main menu loaded`. Must NOT log `HUD created at title screen`. Must NOT hook MainBarGUI ctor `0x72C1E0`. Must NOT walk the Gui tree. Must NOT `setSize` except LifeBar10Green. Must NOT `setVisible` on Root / MedicalPanel / parents / LifeBar1–9. Must NOT call `_getWidget`. Must NOT hunt Widget::setCaption. Must NOT log the rejected v1.23–v1.26 / `46d810a` ready strings.
...
- [ ] **Person-select shows Hemolymph on LifeBar10 (after Hunger).** Select a **person**. LifeBar10 sits on the grey plate (not under the chrome). Hemolymph is on the bar (Datapanel), the number is in the right column (not “…molym”). Green bar is filled. Blood stays Blood. 1–9 same pixel size. `setCapW=0` expected. `setCapISub=1`. After orig: `after show vis10=1`. `painted=1` if vis=1 and Datapanel Hemolymph and Value is a number and green w>0. Door / box / chair: hide or clear LifeBar10* only — never touch LifeBar1. Send `RE_Kenshi_log.txt`.
