...
- [ ] **Person-select shows Hemolymph on LifeBar10 (after Hunger).** Select a **person**. LifeBar1–9 sit in their Dark UI slots (no floating outlines). LifeBar10 sits **in MedicalPanel under Hunger** — not a separate panel, not chopped by the 9-bar dashed cap. **Hemolymph** is on the bar (Datapanel), **99** (or current number) is in the right column — never the word on Value. Green is a **nearly-full** bar at 99/100, not a 1px sliver. Log `parentW` (50–400) / `fillW` / `green w` / Datapanel vis+coord+caption. `painted=1` only if vis=1 and Datapanel Hemolymph and Value is a number and green w is a real bar. Left Leg 5 crippled must log as a stump and tick; 75-HP arms stay 75. Door / box / chair: hide or clear LifeBar10* only. Send `RE_Kenshi_log.txt`.
