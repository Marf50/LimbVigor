...
**Where to look:** select a **character** after In-game. A **new row under Hunger** (LifeBar10) must show **Hemolymph** (or Vigor / Battle-heat). Blood stays Blood and stays in its slot. Head/Stomach/Chest/arms/legs/Hunger stay in their slots (same pixel size). Name plate / Squad / Floor must not drift. Log must bind `Widget::setCaption` (`setCapW=1`), `setVisible` only on LifeBar10 / LifeBar10Value, write every tick, and `getCaption` read-back must be Hemolymph (not Blood / empty). Must NOT call `_getWidget`. Door hides/clears LifeBar10 only. See testdoc/HUD.md.
...
- [ ] **Person-select shows Hemolymph on LifeBar10 (after Hunger).** Select a **person**. Whole HUD stays visible and in slots. Blood stays Blood. You must **see** Hemolymph on the new bottom row (not only a log line). Log `setCapW=1`. Read-back `getCaption` is Hemolymph. `painted=1` only after read-back. Door / box / chair: hide or clear LifeBar10 / LifeBar10Value only — never touch LifeBar1. Send `RE_Kenshi_log.txt`.
