...
**Where to look:** select a **character** after In-game. A **new row under Hunger** (LifeBar10) must show **Hemolymph** (or Vigor / Battle-heat) **on screen** (MedicalPanel slid up). Blood stays Blood and stays in its slot. Head/Stomach/Chest/arms/legs/Hunger stay in their slots (same pixel size). Name plate / Squad / Floor / speed chrome must not drift. Log: `setCapW=0` expected (Widget::setCaption is not exported). `setCapISub=1`. ISub write on Datapanel / `getSubWidgetText(LifeBar10)`. After orig `vis10=1`. `getCaption` Hemolymph (not Blood / empty). `painted=1` only if vis=1 and Hemolymph/Vigor/Battle-heat — not from an empty Value TextBox. Must NOT call `_getWidget`. Door hides/clears LifeBar10* only. MedicalPanel y is `0.68851849555969238`. See testdoc/HUD.md.
...
- [ ] **Person-select shows Hemolymph on LifeBar10 (after Hunger).** Select a **person**. Whole HUD stays visible and in slots. LifeBar10 is on-screen (not below the bottom). Blood stays Blood. You must **see** Hemolymph on the new bottom row (not only a log line). `setCapW=0` expected. `setCapISub=1`. After orig: `after show vis10=1`. Read-back `getCaption` is Hemolymph. `painted=1` only if vis=1 and Hemolymph/Vigor/Battle-heat — not an empty Value. Door / box / chair: hide or clear LifeBar10* only — never touch LifeBar1. Send `RE_Kenshi_log.txt`.
